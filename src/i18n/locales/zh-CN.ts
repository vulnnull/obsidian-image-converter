...
export const zhCN: DeepPartial<TranslationStrings> = {
	...
	// ----------------------------------------------------------------
	commands: {
		processAllVaultImages: "处理库中所有图片",
	...
	notices: {
		// -- 初始化 --
		failedToInitialize: "图片转换器初始化失败，请查看控制台了解详情",

		// -- 无活动文件 --
		noActiveFileDetected: "未检测到当前打开的文件",

		// -- 设置 --
		unableToOpenSettings: "无法打开设置，请检查该插件是否已启用",
		...
		// -- 确认对话框（拖放/粘贴） --
		showPresetSelectionModal: "显示预设选择弹窗？",
		...
		errorIncrementingFilename: '为"{fileName}"自动编号文件名时出错，请查看控制台了解详情',
		...
		failedToProcessImageWithMessage: '处理图片"{fileName}"失败：{errorMessage}，请查看控制台了解详情',
		failedToProcessImageCheckConsole: '处理图片"{fileName}"失败，请查看控制台了解详情',
	...
	contextMenuNotices: {
		...
		// -- 剪切 --
		noActiveMarkdownViewFound: "未找到当前 Markdown 视图",
		...
		// -- 处理/转换 --
		noActiveMarkdownView: "没有当前 Markdown 视图",
		...
		// -- 在导航中显示 --
		failedToShowInNavigation: "无法在导航栏中显示",

		// -- 在系统资源管理器中显示 --
		failedToShowInSystemExplorer: "无法在系统文件管理器中显示",
	...
	batchProcessing: {
		...
		errorInvalidFolderPath: "无效的文件夹路径",
		noImagesFoundInFolder: "文件夹中未找到图片",
		noImagesFoundInVault: "库中未找到图片",
		noProcessingNeededAllVaultSkippedOrOriginal: "无需处理：所有库中图片要么在跳过列表中，要么保持原始格式且无压缩或缩放",
		noProcessingNeededAllVaultAlreadyFormat: "无需处理：所有库中图片要么在跳过列表中，要么已是 {format} 格式且无压缩或缩放",
		noProcessingNeededAllVaultAlreadyInFormat: "无需处理：所有库中图片要么是 {format} 格式，要么在跳过列表中",
		...
		noProcessingNeededAllVaultAlreadyInFormatSkipList: "无需处理：所有库中图片要么是 {format} 格式，要么在跳过列表中",
	},
	...
	settings: {
		...
		neverProcessFilenamesInfo: "逗号分隔的文件名或模式列表，该插件将永不处理这些文件。支持 glob（*）和正则表达式（用 `/`、`r/` 或 `regex:` 括起来）。例如：`old.png, /^_/, r/temp-.*\\.jpg$/`。也可以直接跳过所有猫的图片如：/cat/ 或所有 gif 图片 *.gif",
		showSizeNotification: "显示图片大小变化通知",
		showSizeNotificationInfo: "处理图片后显示节省了多少空间的通知",
		showWindow: "显示窗口",
		...
		defaultAlignmentForNewImagesDesc: "插入新图片时自动应用此对齐方式，设为 none 可禁用",
		imageAlignmentCacheLocation: "图片对齐缓存位置",
		imageAlignmentCacheLocationDesc: "选择图片对齐缓存文件的存储位置，需重启应用生效",
		imageAlignmentCacheLocationInfo: "如果使用 Obsidian Sync，强烈建议所有设备使用相同位置以确保行为一致。默认：Obsidian 的 config 文件夹（可同步）",
		...
		enableDragResizeInfo: "这会在图片下方创建一个新的 <DIV> 来显示缩放手柄，但可能与某些主题不兼容，导致图片跳动",
		...
		disableObsidianImageSelectionOnClickInfo: "在实时预览中点击内部图片时保持编辑器焦点，而非显示 Obsidian 的默认轮廓/缩放角。光标位置遵循拖放/粘贴光标位置设置",
		disableObsidianImageSelectionOnClickDesc: "在实时预览中点击内部图片时保持编辑器焦点，而非显示 Obsidian 的默认轮廓/缩放角。光标位置遵循拖放/粘贴光标位置设置",
		cursorPositionDuringResize: "缩放时光标位置",
		cursorPositionDuringResizeInfo: "调整图片大小时光标放置的位置。注意：「不移动光标」会尝试保持光标在原位，但若拖拽缩放完成时光标仍在图片上方，文本会被选中",
		allowResizingInReadingMode: "允许在阅读模式下缩放",
		allowResizingInReadingModeDesc: "阅读模式下的非破坏性调整大小仅是视觉效果，若觉得太分散注意力可禁用",
		// 图片说明设置
		captions: "图片说明",
		textAlignmentWithinCaption: "图片说明文本对齐",
		textTransform: "文本转换",
		textTransformDesc: "设置文本转换方式",
		fontSize: "字体大小",
		fontSizeDesc: "设置图片说明的字体大小（例如：12px, 1.2em）",
		weight: "字重",
		weightDesc: "设置字体粗细（例如：normal, bold, 600）",
		color: "颜色",
		colorDesc: "选择图片说明的颜色，例如：red, grey, white, black, hsl(50, 50%, 50%), rgb(50%, 75%, 100%)",
		fontStyle: "字体样式",
		fontStyleDesc: "设置字体样式（例如：italic, normal）",
		backgroundColor: "背景颜色",
		backgroundColorDesc: "选择图片说明的背景颜色（例如：transparent, #f5f5f5, rgba(255,255,255,0.8)）",
		border: "边框",
		borderDesc: "设置边框样式（例如：1px solid gray）",
		borderCornerRadius: "边框圆角",
		borderCornerRadiusDesc: "设置图片说明边框圆角（例如，略微圆角：4px）",
		spaceAtTheTop: "顶部间距",
		spaceAtTheTopDesc: "设置图片与图片说明之间的间距（例如：4px, 8px）",
		padding: "内边距",
		paddingDesc: "设置图片说明内边距（例如：4px 8px）",
		skipCaptionExtensions: "跳过图片说明的扩展名",
		skipCaptionExtensionsDesc: "逗号分隔的图片扩展名列表，排除在图片说明之外（例如：PNG, JPG）",
		// 通知消息
		contextMenuDisabled: "右键菜单已禁用。需重启 Obsidian 生效",
		contextMenuEnabled: "右键菜单已启用。需重启 Obsidian 生效",
		imageAlignmentDisabled: "图片对齐已禁用。需重启 Obsidian 生效",
		imageAlignmentEnabled: "图片对齐已启用。需重启 Obsidian 生效",
		imageResizingDisabled: "图片缩放已禁用。需重启 Obsidian 生效",
		imageResizingEnabled: "图片缩放已启用。需重启 Obsidian 生效",
		imageCaptionsDisabled: "图片说明已禁用。需重启 Obsidian 生效",
		imageCaptionsEnabled: "图片说明已启用。需重启 Obsidian 生效",
		// 预设表单
		presetName: "预设名称",
		...
		filenamePresetDesc: "控制转换后图片的命名方式，使用 {notename}、{timestamp}、{uuid} 或 {MD5:filename} 等变量创建唯一文件名",
		conversionPresetDesc: "控制转换图片的输出格式、质量和缩放选项，可显著减小文件大小并保持库体积小巧",
		linkFormatPresetDesc: "确定图片链接如何插入到笔记中，在 Wikilinks 和 Markdown 链接之间选择，并指定文件路径的格式化方式。可为图片使用不同于库默认的链接样式，提供更好的跨应用兼容性",
		resizePresetDesc: "配置编辑器内图片的非破坏性调整大小选项，可调整显示大小而不修改原始文件",
		// 预设组标题
		folderPresets: "文件夹预设",
		...
		executablePathForPngquantInfo: "提供二进制文件的完整路径，可位于库内或文件系统中的任何位置",
		...
		// 默认文件夹/文件名预设名称
		defaultObsidianSetting: "默认（跟随 Obsidian 设置）",
		rootFolder: "根文件夹",
		sameFolderAsCurrentNote: "与当前笔记所在文件夹相同",
		keepOriginalName: "保留原始名称",
		noteNameTimestamp: "笔记名-时间戳",
		// 新增设置字符串
		revertToOriginalIfLargerDesc: "如果处理后的图片文件大小大于原始图片，则使用原始图片。有时压缩会增加文件大小（特别是某些格式或设置下），若希望始终获得更小的文件大小，请启用此选项",
		minimumCompressionSavingsDesc: "指定压缩图片前必须减少多少文件大小。有时图片大小只缩小 3 KB，但质量下降明显。此选项可捕获这些情况并避免压缩。默认 30 KB，即压缩后仅减少 30 KB 或更少时使用原始图片。设为 0 则在输出更小时始终允许压缩",
		...
		longestEdgeDesc: "自动读取原始图片尺寸，将提供的值应用于宽度或高度中较长的一边。若启用了「保持宽高比」，另一边将自动计算",
		shortestEdgeDesc: "自动读取原始图片尺寸，将提供的值应用于宽度或高度中较短的一边。若启用了「保持宽高比」，另一边将自动计算",
		customSizeDesc: "使用 |宽x高 格式（例如 300x200）或百分比格式（例如 50x75）同时设置宽度和高度，此模式不保持宽高比",
		maxWidthDesc: "设置图片的最大宽度以适应编辑器宽度，可指定百分比或固定像素值",
		...
		no: "否",
		// 预设摘要 - 文件夹
		folderSummaryDefault: "默认（使用 Obsidian 配置的附件设置）",
		folderSummaryRoot: "库的根目录（顶层文件夹）",
		folderSummaryCurrent: "与当前正在编辑的笔记所在文件夹相同",
		folderSummarySubfolder: "在子文件夹中：{template}",
		folderSummaryCustom: "自定义位置：{template}",
		folderSummaryUnknown: "未知位置",
		// 预设摘要 - 示例
		exampleLoading: "示例：加载中…",
		exampleValue: "示例：{value}",
		exampleError: "示例：生成预览时出错",
		// 预设摘要 - 文件名
		templateValue: "模板：{value}",
		skipRenamePatternsValue: "跳过重命名模式：{value}",
		conflictResolutionValue: "如果输出文件已存在：{value}",
		// 预设摘要 - 链接格式
		linkFormatSummary: "链接类型：{linkFormat}，路径类型：{pathFormat}",
		// 预设摘要 - 转换
		formatValue: "格式：{format}",
		qualityValue: "质量：{quality}",
		colorDepthValue: "色深：{colorDepth}",
		ffmpegCrfValue: "FFmpeg CRF：{crf}",
		ffmpegPresetValue: "FFmpeg 预设：{preset}",
		resizeValue: "缩放：{resizeMode}",
		widthValue: "（宽度：{width}）",
		heightValue: "（高度：{height}）",
		longestEdgeValue: "（最长边：{value}）",
		shortestEdgeValue: "（最短边：{value}）",
		enlargeReduceValue: "放大/缩小：{value}",
		allowLargerFilesValue: "允许更大的文件：{value}",
		revertToOriginalYes: "如果更大则还原为原始：是",
		// 预设摘要 - 缩放
		noResizing: "无缩放",
		originalWidth: "原始宽度",
		originalHeight: "原始高度",
		widthPx: "宽度：{value}",
		heightPx: "高度：{value}",
		customSizeValue: "自定义：{value}",
		longestEdgePx: "最长边：{value}",
		shortestEdgePx: "最短边：{value}",
		editorMaxWidthPx: "编辑器最大宽度：{value}",
		scaleModeValue: "缩放模式：{value}",
		respectEditorMaxWidthValue: "不超过编辑器最大宽度：{value}",
		maintainAspectRatioValue: "保持宽高比：{value}",
		// 缩放下拉选项
		resizeModeNone: "无",
		resizeModeFit: "适应",
		resizeModeFill: "填充",
	...
	imageProcessor: {
		...
		pngquantPathNotSet: "未设置 pngquant 可执行文件路径，请在插件设置中配置",
		ffmpegPathNotSet: "未设置 FFmpeg 可执行文件路径，请在插件设置中配置",
	...
	processModals: {
		...
		couldNotFindRenamedFile: "在 {fullPath} 找不到重命名后的文件",
		linkUpdatedIn: '"{fileName}" 中的链接已更新',
		imageProcessedButFailedToRefresh: "图片已处理，但无法刷新视图，可能需要重新加载笔记",
		...
		ffmpegExecutablePath: "FFmpeg 可执行文件路径",
		ffmpegExecutablePathInfo: "提供 FFmpeg 二进制文件的完整路径，可位于库内或文件系统中的任何位置",
		encoderDetection: "编码器检测",
		encoderDetectionInfo: "自动检测 FFmpeg 构建中可用的 AV1 编码器",
		detectEncoder: "检测编码器",
		ffmpegCrf: "FFmpeg CRF",
		ffmpegCrfInfo: "恒定速率因子（0-63），值越低质量越好、文件越大，默认：23",
		ffmpegPreset: "FFmpeg 预设",
		ffmpegPresetInfo: "编码速度/质量权衡，越慢压缩越好",
		...
		warningModifiesImages: "⚠️ 这将修改所选文件夹及子文件夹（若启用了递归）中的所有图片，请确保已备份",
		...
		errorActiveFileMustBeMarkdownOrCanvas: "当前文件必须是 Markdown 或画布文件",
		// --- ProcessAllVaultModal specific ---
		convertCompressResizeAllImages: "转换、压缩和缩放所有图片",
		inTheVault: "在整个库中",
		warningModifiesAllVault: "⚠️ 这将修改整个库中的所有图片，请确保已备份",
		...
		skipTargetFormatDesc: "若图片已是目标格式，则跳过压缩/缩放",
		skipTargetFormatTooltip: "若图片已是目标格式，允许跳过其压缩、转换和缩放，仍将处理所有其他格式",
	...
	folderAndFilename: {
		...
		inconsistentFolderCasing: "检测到文件夹大小写不一致，使用原始路径：{currentPath}",
		temporaryFileNotFoundAfterRenaming: "重命名后找不到临时文件",
